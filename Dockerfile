...
FROM zoeyvid/nginx-quic:536-python
SHELL ["/bin/ash", "-eo", "pipefail", "-c"]
COPY rootfs /
COPY src    /app/src

COPY --from=zoeyvid/valkey-static:60 /usr/local/bin/valkey-cli    /usr/local/bin/valkey-cli
COPY --from=zoeyvid/valkey-static:60 /usr/local/bin/valkey-server /usr/local/bin/valkey-server
