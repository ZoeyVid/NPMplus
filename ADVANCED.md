# NPMplus advanced reference

This is the original detailed reference for manual Compose deployments and expert configuration. Most users should start with the simple instructions in [README.md](README.md).
...
- many other things, see this advanced reference and the compose.yaml
...
- integrated security view in the admin UI ("CrowdSec" page, admin only): current decisions and Anubis honeypot bans, a scenario attack-mix donut with a per-interval activity strip, geolocation, count-aware scenario/country/ASN/IP/target filters, bounded server-side alert-history pagination, a dedicated AppSec WAF tab with configuration state and inspected/passed/blocked traffic, a WAF verdict card and bouncer decision-hit counts on the overview (bans actually served to the proxy, not just collected), CrowdSec Prometheus summaries, and opt-in browser alerts for attack spikes, active-ban changes, or LAPI outages while the page is open. It queries the existing CrowdSec LAPI directly and creates no second dashboard service or database. Unban (deletes the exact decision and lands in the audit log), manual ban, historical alert details, and per-ban context use a LAPI machine account because bouncer keys are read-only. Needs `/data/crowdsec/lapi-ui.key` and `/data/crowdsec/lapi-ui-machine.key`; `setup-npmplus.sh` creates and heals both. Prometheus is read on the private Docker network at `http://crowdsec:6060/metrics` and is published only to host loopback for host-network compatibility; metrics failure degrades only that panel.
...
- daily crowdsec key-heal cron: verifies the UI bouncer, UI machine, nginx bouncer, and host firewall bouncer keys against the LAPI every day and re-registers any the LAPI rejects, so a sqlite rollback after an unclean shutdown never leaves the ban view broken, bans unenforced, or the protected boot gate closed after a reboot; after healing the host firewall bouncer it re-attempts the protected startup so the public ports reopen on their own (log: `/var/log/npmplus-crowdsec-heal.log`)
